...
//
// FIXED: Hapus DOMContentLoaded listener — inisialisasi dipusatkan
// di reservation.js supaya tidak ada race condition.
// ======================================================
...
function renderTableHeader(){
    ...
    if(!colgroup || !headerRow){
        console.warn("[tableColumns] reservationColgroup atau reservationHeaderRow tidak ditemukan");
        return;
    }
    ...
    state.visibleOrder.forEach(key => {
        ...
        // ---- resize handle: Pointer Events + capture ----
...
// ======================================================
// Modify Table Popup — draft state
// ======================================================
...
function buildColumnListItem(key, listName){
    ...
    li.addEventListener("click", () => moveColumnBetweenLists(key, listName));
...
function setupListContainerDragDrop(ulElement, listName){
    if(!ulElement) return;
    ulElement.addEventListener("dragover", (e) => { e.preventDefault(); });
...
function savePresetFromInput(){
    ...
    savePresets(presets);
    showMessage(isReplace ? `Preset "${nameRaw}" diperbarui` : `Preset "${nameRaw}" disimpan`, "success");
    populatePresetDropdown();
}
...
// ======================================================
// Init — DIPANGGIL DARI reservation.js, bukan DOMContentLoaded sendiri
// ======================================================
