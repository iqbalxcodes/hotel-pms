...
const GUEST_COLUMN_LABELS = {
    ...
    // loyalty_level SENGAJA gak masuk sini — itu nama tier (FK ke
    // loyalty_tiers), gak bisa diedit sebagai teks bebas lagi.
};

function setupCheckbox(){
    document.querySelectorAll(".guest-checkbox").forEach(box => {
...
function updateActionBar(){
    ...
    if(selectAll) selectAll.checked = selected.length === all.length && all.length > 0;
...
function toggleAllCheckbox(master){
    document.querySelectorAll(".guest-checkbox").forEach(box => { box.checked = master.checked; });
    updateActionBar();
}
...
function enableEdit(column, inputType = "text"){
    ...
    if(column === "loyalty_level"){
        showMessage("Loyalty tier belum ada UI pilih — perlu dropdown tier terpisah", "error");
        return;
    }
    ...
    cells.forEach(cell => {
        ...
        cell.innerHTML = `<input type="${inputType}" class="edit-input" data-column="${column}" data-id="${id}" value="${value}">`;
    });
...
async function saveEdit(column){
    ...
    // guest_list_view itu view (join ke loyalty_tiers) — update harus
    // langsung ke tabel guests, dengan nama kolom aslinya.
    const dbColumn = {
    ...
    for(const input of inputs){
        ...
        if(column === "loyalty_points") value = value === "" ? 0 : Number(value);

        const { error } = await supabaseClient
            .from("guests")
            .update({ [dbColumn[column]]: value })
...
function resetHeader(column){
    ...
    if(column === "loyalty_level"){
        // read-only, cuma sort, gak ada pencil
        document.getElementById(`${column}Header`).innerHTML = `Loyalty ↕`;
        document.getElementById(`${column}Header`).setAttribute("onclick", `sortTable('${column}')`);
        return;
    }

    const inputType = column === "loyalty_points" ? "number" : column === "email" ? "email" : "text";

    document.getElementById(`${column}Header`).innerHTML = `
        ${GUEST_COLUMN_LABELS[column]} ↕
        <button onclick="event.stopPropagation(); enableEdit('${column}','${inputType}')">✏️</button>
    `;
...
function renderPaginationBar(){
    ...
    info.innerText = totalCount > 0 ? `${totalCount} clients · Page ${currentPage}/${totalPages}` : "No clients";
